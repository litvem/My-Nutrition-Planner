<template>
      ...
      <div class="row">
        ...
        <div class="form-body sm">
          <div class="form-holder sm">
            ...
            <div class="form-items sm" v-if="user">
                <div class="editWeeks row-sm-3 d-flex flex-row mt-2 mb-2 ml-2 align-items-center gap-2">
                  <label id="day" for="Year"> Year: </label>
                  <input class="form-control" type="text" v-if="!deleteDay" :disabled="!deleteDay" :class="{view: !deleteDay}" :value="this.$route.params.year">
                  <input type="text" v-if="editYear" v-model="year" id="thisYear" name="year" class="form-control"/>
                  <button class="btn edit-btn btn-sm" @click="deleteDay = !deleteDay" v-if="!deleteDay">&nbsp;Edit</button>
                </div>
                <div class="editWeeks col-sm-12 d-flex flex-row mt-2 mb-2 ml-5 align-items-center gap-2" v-if="deleteDay">
                  <button class="btn save-btn btn-sm " @click="deleteDay()">Save</button>
                  <button class="btn cancel-btn btn-sm"  @click="deleteDay=!deleteDay">Cancel</button>
                </div>
              </div>
            </div>
            <!--- options -->
        ...
        <!--alert if weekly calendar already exists-->
            <div class="alert alert-warning d-flex align-items-center" :show="dismissCountDown" variant="success" @dismissed="dismissCountDown=0"
            @dismiss-count-down="countDownChanged"  role="alert" v-if="recipesExist">
              <svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Warning:"><use xlink:href="#exclamation-triangle-fill"/></svg>
                This day has no recipes
            </div>
          </div>
        </div>
  ...
  <!---------------SÖNDAG-----------> @click="deleteRecipe(sunRecipes._id)"
            <div class="day">
              <div class="day-week">
...
export default {
  ...
  methods: {
    ...
    deleteRecipe(id) {
      const day = this.days.map(day => day.recipes.map(recipe => recipe._id === id))
      console.log(day)

      const currentDay = this.days.find(day => {
        return day.recipes.some((recipe) => {
          return recipe._id === id
        })
      })
      console.log(currentDay)

      for (const recipe of this.days.recipes) {
        if (day._id === id) {
          const index = recipe.findIndex(recipe => {
            return recipe._id === id
          })
          console.log(this.days.recipes[index])
          this.days.recipes.splice(index, 1)
        }
      }

      Api.patch('/profiles/' + localStorage.id + '/days/' + day._id, {
        recipes: this.days.recipes
      },
      {
        headers: {
          Authorization: 'Bearer ' + localStorage.token
